...
# used to track the location of this tool
            ...
            for priv_key in private_keys:
                if priv_key is not None:
                    ...
                    if not str(priv_key):
                        ...
                        if priv_key.d is not None:
                            logger.info(f"d: {priv_key.d}")
                        if args.c is not None and priv_key.d is not None and priv_key.n is not None:
                            logger.info(
                                "Decrypting ciphertext with found private key..."
                            )
                            
                            m = pow(args.c, priv_key.d, priv_key.n)
                            decrypt = n2s(m)
                            print_decrypted_res(decrypt, logger)
